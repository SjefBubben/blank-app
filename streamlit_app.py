...
def fetch_supabase_konsum_data():
    ...
    try:
        ...
        df['datetime'] = pd.to_datetime(df['datetime'].strftime('%d.%m.%y %H:%M'), errors='coerce')
...
def map_konsum_to_games_and_save(konsum_df, games_df, hours_window=12):
    ...
    games_df["game_finished_at"] = pd.to_datetime(games_df['game_finished_at'].strftime('%d.%m.%y %H:%M'), errors="coerce")
    games_df = games_df.dropna(subset=['game_finished_at'].strftime('%d.%m.%y %H:%M'))
