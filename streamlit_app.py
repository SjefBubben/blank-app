                ...
                with col2:
                    st.markdown(f"""
                        <div style="padding: 15px; background-color: #F44336; color: white; border-radius: 10px; text-align: center;">
                            ...
                            <h4>Reaksjonstid: {', '.join(p["reactionTime"] for p in low_players)} - {details["mapName"]} <br> ({datetime.strptime(details["finishedAt"], "%Y-%m-%dT%H:%M:%S.%fZ").strftime("%d.%m.%Y %H:%M")})</h4>
                        </div>
                    """, unsafe_allow_html=True)
